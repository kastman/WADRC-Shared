class AppointmentsController < ApplicationController
  ...
  def show
    ...
    @series = @appointment.series.order("`series_set_id` ASC, `order` ASC")#.with_sequence_set #.with_pulses_or_tasks.includes(:series_metainfo, :series_log_items => {:functional_scenario => :functional_set}).order("`pfile` ASC, `order` ASC") # => {:functional_scenario => :functional_set})#.order("functional_sets.setname, series.order")
